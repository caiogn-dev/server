...
async function login(username, password) {
  const response = await fetch('http://localhost:12000/api/login/', {
    method: 'POST',
...
async function fetchProfile() {
  ...
  const response = await fetch('http://localhost:12000/api/users/profile/', {
...
async function register(userData) {
  const response = await fetch('http://localhost:12000/api/users/register/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...
      first_name: userData.firstName,  // optional
...
### Add Token to All Requests (Axios Interceptor)
...
async function createCheckout(formData) {
  const response = await api.post('/checkout/create_checkout/', {
    buyer: {
      name: formData.name,           // Required: Full name
      email: formData.email,         // Required: Valid email
      phone: formData.phone,         // Required: 10-11 digits (Brazilian)
      cpf: formData.cpf,             // Optional: 11 digits (Brazilian CPF)
      address: formData.address,     // Required: Street, number, neighborhood
      city: formData.city,           // Required: City name
      state: formData.state,         // Required: 2-letter state code (SP, RJ, etc.)
      zip_code: formData.zip_code    // Required: 8 digits (Brazilian CEP)
    }
  });
...
async function handleCheckout() {
  try {
    const result = await createCheckout({
      ...
      phone: '11987654321',      // Numbers only
      cpf: '12345678901',        // Numbers only
      address: 'Rua Principal, 123, Centro',
      city: 'São Paulo',
      state: 'SP',
      zip_code: '01234567'       // Numbers only
    });
