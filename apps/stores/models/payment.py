...
class StorePaymentGateway(BaseModel):
    ...
    api_key_encrypted = models.TextField(blank=True, help_text='API Key (criptografado)')
    api_secret_encrypted = models.TextField(blank=True, help_text='API Secret (criptografado)')
    access_token_encrypted = models.TextField(blank=True, help_text='Access Token (criptografado)')
    webhook_secret_encrypted = models.TextField(blank=True, help_text='Webhook Secret (criptografado)')
    public_key_encrypted = models.TextField(blank=True, help_text='Public Key (criptografado)')
    ...
    def _get_credential(self, encrypted_field_name: str) -> str:
        value = getattr(self, encrypted_field_name, '')
        if not value:
            return ''
        try:
    ...
    def _set_credential(self, encrypted_field_name: str, value: str):
        if value:
            setattr(self, encrypted_field_name, token_encryption.encrypt(value))
        else:
            setattr(self, encrypted_field_name, '')
