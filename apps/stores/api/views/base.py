...
class IsStoreOwnerOrStaff(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if not (request.user and request.user.is_authenticated):
            logger.warning("[IsStoreOwnerOrStaff] User not authenticated")
            return False

        logger.debug("[IsStoreOwnerOrStaff] User authenticated: id=%s is_staff=%s is_superuser=%s", request.user.pk, request.user.is_staff, request.user.is_superuser)
