...
# ============================================================================
# Combo Serializers - ComboProductGroup and ComboProductGroupVariantLimit
# ============================================================================
...
class StoreProductVariantBasicSerializer(serializers.ModelSerializer):
    """Lightweight serializer for product variants in combo context."""
    ...
    class Meta:
        ...
        fields = [
            ...
            'price', 'stock_quantity', 'product_track_stock'
        ]
        read_only_fields = [
            'id', 'name', 'product_id', 'product_name',
            'price', 'stock_quantity', 'product_track_stock'
...
class ComboProductGroupVariantLimitSerializer(serializers.ModelSerializer):
    """Serializer for per-variant selection limits within a combo group."""
    ...
    class Meta:
        ...
        fields = [
            'id', 'variant_id', 'variant_name', 'stock',
            'max_selections', 'price_override',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'variant_id', 'variant_name', 'stock', 'created_at', 'updated_at']


class ComboProductGroupSerializer(serializers.ModelSerializer):
    """
    Serializer for product groups within a combo.

    Includes nested variant limits and selection rules.
    """
    ...
    # Nested variant limits
    variant_limits = ComboProductGroupVariantLimitSerializer(
        many=True,
        read_only=True
    )
    ...
    class Meta:
        ...
        fields = [
            'id', 'combo', 'product_id', 'product_name', 'product_slug',
            'is_required', 'min_selections', 'max_selections',
            'allow_duplicate_variants', 'position',
            'variant_limits',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
...
class ComboDetailSerializer(serializers.ModelSerializer):
    """
    Comprehensive serializer for combo bundles with full nested structure.

    Includes:
    - Combo basic info and pricing
    - All product groups with selection rules
    - Per-group variant limits and available variants
    """
    ...
    def get_image_url_full(self, obj):
        """Return full image URL for the combo."""
        return obj.get_image_url()


class ComboListSerializer(serializers.ModelSerializer):
    """
    Lightweight serializer for listing combos (no nested groups).

    Used for list endpoints where full detail is not needed.
    """
    ...
    def get_groups_count(self, obj):
        """Return count of product groups in combo."""
        return obj.groups.count()


class ComboCreateUpdateSerializer(serializers.ModelSerializer):
    """
    Serializer for creating and updating combos.

    Handles write operations with validation.
    """
    ...
    def validate_compare_at_price(self, value):
        """Ensure compare_at_price is greater than price if set."""
        if value is not None and value <= 0:
            raise serializers.ValidationError(
                "Preço de comparação deve ser maior que zero."
            )
        return value
    ...
    def validate(self, data):
        ...
        if self.instance:
            ...
            if compare_at_price is None:
                compare_at_price = self.instance.compare_at_price
        ...
        if compare_at_price and price:
            if compare_at_price < price:
                raise serializers.ValidationError({
                    'compare_at_price': (
                        "Preço de comparação deve ser maior ou igual ao preço "
                        "do combo."
                    )
                })
    ...
    def create(self, validated_data):
        """Create new combo with store from context."""
        store = self.context['request'].user.stores.first()
        if not store:
            raise serializers.ValidationError("Usuário não tem loja associada.")
