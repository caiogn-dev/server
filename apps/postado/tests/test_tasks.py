...
class TestGeneratePackTask(TestCase):
    ...
    def _make_drive_mock(self, mock_drive_cls):
        ...
        mock_drive.create_subfolder_with_url.return_value = (
            'month_folder_id', 'https://drive.google.com/month_folder'
        )
    ...
    def _make_img_mock(self, mock_img_cls):
        mock_img = MagicMock()
    ...
    def test_generate_pack_sets_generating_then_review(self, mock_copy_cls, mock_img_cls, mock_drive_cls):
        ...
        self._make_img_mock(mock_img_cls)
        mock_drive = self._make_drive_mock(mock_drive_cls)
        mock_drive.create_subfolder_with_url.return_value = ('sfid', '')

        with patch.object(PostadoPack, 'save', track_save):
            generate_pack(str(self.pack.id))

        self.assertEqual(statuses[0], PostadoPack.Status.GENERATING)
        self.assertEqual(statuses[-1], PostadoPack.Status.REVIEW)
    ...
    @patch('apps.postado.tasks.ImageService')
    @patch('apps.postado.tasks.CopyService')
    def test_generate_pack_drive_folder_url_saved(self, mock_copy_cls, mock_img_cls, mock_drive_cls):
        """Pack.drive_folder_url is populated from the month folder."""
        mock_copy_cls.return_value.generate.return_value = {
            'caption': 'c', 'cta': 'cta', 'hashtags': '#h',
        }
        self._make_img_mock(mock_img_cls)
        self._make_drive_mock(mock_drive_cls)
    ...
    @patch('apps.postado.tasks.ImageService')
    @patch('apps.postado.tasks.CopyService')
    def test_generate_pack_all_posts_fail_resets_to_pending(self, mock_copy_cls, mock_img_cls, mock_drive_cls):
        """If every post fails, pack status must be reset to PENDING, not REVIEW."""
        mock_copy_cls.return_value.generate.side_effect = RuntimeError("copy failed")
        self._make_img_mock(mock_img_cls)
        self._make_drive_mock(mock_drive_cls)
        ...
        self.pack.refresh_from_db()
        self.assertEqual(self.pack.status, PostadoPack.Status.PENDING)
        # Posts are created in the DB but none reach GENERATED status
        self.assertTrue(all(
            p.status != PostadoPost.Status.GENERATED
            for p in self.pack.posts.all()
        ))
