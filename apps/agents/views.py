...
class AgentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser or user.is_staff:
            return qs
        return qs.filter(accounts__owner=user).distinct()
...
class AgentConversationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        qs = AgentConversation.objects.filter(agent__is_active=True).order_by('-last_message_at')
        if user.is_superuser or user.is_staff:
            return qs
        return qs.filter(agent__accounts__owner=user).distinct()
