...
class WhatsAppAuthService:
    ...
    def _get_template_data(cls, code: str) -> dict:
        ...
        IMPORTANTE:
        - Marketing templates: formato simples com body parameters
        - Authentication templates: formato específico com copy_code button
        
        Se receber erro #132018, verifique:
        1. O template existe no Meta Business Manager
        2. O template está aprovado
        3. Os parâmetros correspondem às variáveis do template
        """
        if cls.USE_FALLBACK:
            # Template de Marketing (formato mais simples)
            # Este formato é compatível com a maioria dos templates de texto
            return {
        ...
        # Formato correto conforme documentação Meta Cloud API:
        # https://developers.facebook.com/docs/whatsapp/cloud-api/guides/send-message-templates
        #
        # Para templates de autenticação com botão de cópia OTP:
        # - O body geralmente não tem parâmetros (código já está no botão)
        # - O botão copy_code recebe o OTP como parâmetro
        return {
            ...
            'components': [
                ...
                {
                    'type': 'button',
                    'sub_type': 'url',  # 'url' para botões de URL, ou omitir para copy_code simples
                    'index': 0,
                    'parameters': [
                        {'type': 'text', 'text': code}
                    ]
    ...
    def send_auth_code(
        ...
        # DEBUG: Log do UUID
        logger.info(f"[WHATSAPP AUTH DEBUG] UUID received: '{whatsapp_account_id}' (len={len(whatsapp_account_id)})")
