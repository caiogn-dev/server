...
class WhatsAppAuthService:
    ...
    # Desabilitado: texto simples não é canal verificado e viola o contrato
    # de autenticação. Fora da janela de 24h a Meta bloqueia mensagens
    # não-template de qualquer forma. Ver CLAUDE.md.
    ...
    def send_auth_code(
        ...
        logger.debug("[WHATSAPP AUTH] Generated OTP for %s", clean_phone)
        ...
        logger.debug("[WHATSAPP AUTH] Trying %d template config(s) for account %s", len(template_configs), whatsapp_account_id)
        ...
        try:
            ...
            import traceback
            logger.error("[WHATSAPP AUTH] Failed to initialize MessageService: %s\n%s", init_error, traceback.format_exc())
        ...
        for i, template_data in enumerate(template_configs):
            templates_tried += 1
            logger.debug("[WHATSAPP AUTH] Attempt %d/%d: template '%s'", i + 1, len(template_configs), template_data['name'])
            ...
            try:
                ...
                logger.info("[WHATSAPP AUTH] OTP enviado via template '%s'", template_data['name'])
                ...
                logger.error(
                    "[WHATSAPP AUTH] Template '%s' falhou — %s (code=%s): %s\n%s",
                    template_data['name'], error_type, error_code, final_error_str, error_traceback,
                )

                last_error = e
                last_error_details = error_details

                # Erros relacionados ao template: tenta próxima configuração
                # 131008 (required param missing), 132018 (param issue), 131009 (not found), 132000 (wrong count)
                ...
                if any(ec in all_error_text for ec in error_codes_to_retry):
                    logger.warning("[WHATSAPP AUTH] Erro de template (code=%s), tentando próxima config...", error_code)
                    continue
                else:
                    logger.warning("[WHATSAPP AUTH] Erro não relacionado a template (%s=%s), abortando", error_type, error_code)
                    break
        ...
        logger.error("[WHATSAPP AUTH] Todas as tentativas falharam (%d templates). Último erro: %s", templates_tried, error_message)
